...
def slice_train_dataset(df_X, df_target, sp=None):
    ...
    np.save(op.join(dest_dir, 'study_period_X_'+str(sp)+'train.npy'), X_train)
    np.save(op.join(dest_dir, 'study_period_Y'+str(sp)+'_train.npy'), Y_train)
...
def slice_test_dataset(df_X, df_target, dest_dir, sp):
    ...
    if not op.exists(inference_dir):
...
returns=[]   
for firm in tickers:
    df_firm = web.DataReader(firm, "yahoo", start="1990-01-01")
    df_firm["Return"] = df_firm["Close"].pct_change()
    df_firm = df_firm[["Return"]]
    df_firm = df_firm.rename(columns={"Return": "{}".format(firm)})
...
while count+1000 < len(df):
    print("Processing Split :"+str(j+1))
